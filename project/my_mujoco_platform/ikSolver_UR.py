...
class ikSolver():
    ...
    def nearestQ(self, q_list, last_q):
        ...
        for q in q_list:
            if np.isnan(q).any():
                ...
                qregu = np.zeros(6)
                for i in range(6):
                    qregu[i] = self.regulate_toward_qlast(q[i], last_q[0][i])
                confDist = np.sum(((qregu - last_q) * weights) ** 2)
    ...
    def regulate_toward_qlast(self, qnew, qlast):
        diff_rad = qlast - qnew
        if abs(diff_rad) > np.pi:
            qregu = qnew + 2 * np.pi * np.sign(diff_rad)
        else:
            qregu = qnew
